...
from libs.data_retriever import OCTHDF5Dataset
...
from libs.custom_loss import MultiHeadFlippedLossMSE, MultiHeadFlippedLossKL


def main():
    assert loss_flip in ['kl', 'mse'], 'Loss flip must either be KL or MSE'
    N_PIXEL_H = 496
    N_PIXEL_W = 512
    ...
    tb_path = Path(__file__).resolve().parents[0].joinpath('runs/TL_{}_PE'.format(current_time))
    ...
    with open(Path(__file__).resolve().parents[0].joinpath('runs', 'info.txt'), 'a') as file:
        file.write(f'{current_time}: {N_EPOCHS} epochs, LR: {LR}, Columns: {num_columns} Lambda: {lambda_flip}\n')
    ...
    minibatch_size = u.max_minibatch_size(model_name) // 2
    ...
    model = MultiHeadEfficientNet_PE.from_name(num_columns, model_name, num_classes=3)
    ...
    if loss_flip == 'kl':
        srf_criterion = MultiHeadFlippedLossKL(num_columns, lambda_kl=lambda_flip)
        irf_criterion = MultiHeadFlippedLossKL(num_columns, lambda_kl=lambda_flip)
    else:
        srf_criterion = MultiHeadFlippedLossMSE(num_columns, lambda_mse=lambda_flip)
        irf_criterion = MultiHeadFlippedLossMSE(num_columns, lambda_mse=lambda_flip)
    ...
    for epoch in range(N_EPOCHS):
        for phase in ['train', 'validation']:
            ...
            for i, data in enumerate(loader):
                ...
                if phase == 'validation':
                    # Healthy, SRF and IRF
                    labels_cut = torch.index_select(labels, 1, torch.as_tensor([0, 1, 2]).to(device)).float()
                    slices = torch.argmax(slices, dim=2)
                    print(slices.shape, centers.shape)
                    rel_center = torch.abs(slices - centers).squeeze(1).to(device)
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    flip_bio, flip_columns = model(torch.flip(inputs, (3,)), rel_center)

                    y_bio = y_bio[:, :3].to(device)
                    y_srf = y_columns[:, :, 1].to(device)
                    y_irf = y_columns[:, :, 2].to(device)

                    flip_srf = flip_columns[:, :, 1].to(device)
                    flip_irf = flip_columns[:, :, 2].to(device)

                    loss_biom = main_criterion(y_bio, labels_cut)
                    loss_srf = srf_criterion(y_srf, flip_srf, labels_cut[:, 1])
                    loss_irf = irf_criterion(y_irf, flip_irf, labels_cut[:, 2])
                    loss = loss_biom + loss_srf + loss_irf
                ...
                with torch.no_grad():
                    mse_srf = nn.functional.mse_loss(y_srf, torch.fliplr(flip_srf))
                    mse_irf = nn.functional.mse_loss(y_irf, torch.fliplr(flip_irf))
                    mse = mse_srf + mse_irf
                ...
                if i % writing_freq == (writing_freq - 1):
                    ...
                    epoch_mse = running_mse / (writing_freq * minibatch_size)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-lflip', '--lambda-flipped',
                        default=1.,
                        type=float,
                        help='Lambda value for flipped image in loss function')

    parser.add_argument('-lossflip', '--loss-flipped',
                        default='kl',
                        type=str,
                        help='Type of loss function for flipped images (kl or mse)')
    ...
    lambda_flip = FLAGS.lambda_flipped
    loss_flip = FLAGS.loss_flipped.lower()
