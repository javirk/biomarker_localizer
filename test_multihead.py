...
def generate_labels():
    ...
    num_classes = 10 if remove_volumes else 3

    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')

    tifs = data_path.joinpath('extractedTifs')
    labels_path = data_path.joinpath('BSCAN')
    volumes_remove_path = data_path.joinpath('..', 'Segmentation', 'random_sample_reviewed.txt') if remove_volumes else None
    ...
    dataset = OCTSliceMaskDataset(mode='all', image_set=tifs,
                                  ...
                                  encoding_type='noencoding', volumes_remove_file=volumes_remove_path)

    dataloader = torch.utils.data.DataLoader(dataset, batch_size=minibatch_size, shuffle=False, num_workers=1)

    model = MultiHeadEfficientNet.from_name(num_columns, model_name, num_classes=num_classes)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-v', '--remove-volumes',
                        action='store_true',
                        help='File with volumes to remove')
    ...
    remove_volumes = FLAGS.remove_volumes
