...
from libs.custom_loss import CustomLoss
...
def main():
    ...
    with open(Path(__file__).resolve().parents[0].joinpath('runs', 'info.txt'), 'a') as file:
        file.write(f'{current_time}: {N_EPOCHS} epochs, LR: {LR}, Lambda: {lambda_value}\n')
    ...
    t_val = transforms.Compose([CLAHE(), transforms.ToTensor(),
                                 transforms.Normalize([0.5, 0.5, 0.5], [0.5, 0.5, 0.5])])
    ...
    srf_criterion = CustomLoss(num_columns)
    irf_criterion = CustomLoss(num_columns)
    ...
    for epoch in range(N_EPOCHS):
        for phase in ['train', 'validation']:
            ...
            for i, data in enumerate(loader):
                ...
                if phase == 'validation':
                    labels_cut = torch.index_select(labels, 1, torch.as_tensor([0, 1, 2]).to(device)).float() # Healthy, SRF and IRF
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    loss_biom = main_criterion(y_biom, labels_cut)
                ...
                if phase == 'validation':
                    ...
                    l = u.join_labels(y_biom[:, :3], y_srf, y_irf)
        ...
        torch.save(model.state_dict(), Path(__file__).parents[0].joinpath('weights', f'weights_{current_time}_e{epoch}.pth'))
